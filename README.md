...
**소개 페이지**(로그인 불필요) · https://itsms.vercel.app/about
**앱 접속** · https://itsms.vercel.app (사내 계정 로그인 필요)
...
### 3. AI 도우미 (RAG)
올린 문서와 사내 DB만 근거로 답합니다(현재 학습 자료 265조각 — 사내 DB 4종 264조각 + 올린 문서 1조각). **자료에 없으면 지어내지 않고 "문서에 없습니다"라고만 답합니다.** 근거 문서 제목을 출처로 함께 보여줍니다.

```
Q. 자산번호 IFH01990 은 누가 쓰고 월 렌탈료는 얼마인가요?
A. 홍길동님이 사용 중이며, 월 렌탈료는 103,000원입니다.   [출처: 사내 DB · OA 렌탈 자산 목록]

Q. 우리 회사 창립기념일은 언제인가요?
A. 문서에 없습니다.
```

<img src="docs/images/chatbot.png" alt="AI 도우미 채팅창" width="330">

사내 공지 PDF를 올린 뒤 물어보면 그 문서를 근거로 답하고, 근거가 된 자료 이름을 출처로 함께 보여줍니다. 어떤 화면에서든 오른쪽 아래 버튼으로 열리며, 페이지는 그대로 둔 채 대화만 스크롤됩니다.
...
일반 사용자는 개인 자산 현황만, 담당자는 조직 자산 현황까지, 관리자는 관리자 메뉴 전체가 보입니다. 자주 보는 화면은 주황색, 아직 준비 중인 메뉴는 흐리게 표시했습니다.

</td>
<td valign="top"><img src="docs/images/sidebar.png" alt="역할별 사이드바 메뉴" width="220"></td>
<td width="340" valign="top">

**휴대폰 바코드 실사**

자산 태그를 비추면 화면이 멈추고 자산번호·모델코드가 잡히며 사용자 정보가 자동으로 채워집니다. 실제 사용자가 다르면 이름을 눌러 고친 뒤 제출합니다.

</td>
<td valign="top"><img src="docs/images/audit-mobile.jpg" alt="휴대폰 바코드 실사 화면" width="260"></td>
</tr>
</table>

### 모든 OA 현황 — 전사 장비가 어디에 몰려 있나

전사 1,010대를 종류·부서·브랜드로 나눠 봅니다. 막대에 마우스를 올리면 대수·비율·월 렌탈료가 함께 뜨고, 아래 표에서는 자산번호·사용자·부서로 검색해 개별 장비까지 찾아갑니다.

![모든 OA 현황 화면](docs/images/oa-overview.png)

### OA Report & Insight — 돈이 보이는 화면

장비 종류별로 대수와 월 비용이 얼마나 어긋나는지(노트북 465대가 전체 비용의 83%), 부서별 1인당 비용이 유난히 높은 곳은 어디인지 보여줍니다. 위쪽에는 퇴사자 명의·계약종료 잔존 장비를 추려 **회수하면 얼마가 절감되는지**를 먼저 띄웁니다.

![OA Report & Insight 화면](docs/images/oa-report.png)

### License 관리 — 구독 수량 대비 사용률

구독한 21종의 사용률을 한눈에 보여주고, 할당률 90% 이상인 라이선스를 따로 세어 추가 구매 시점을 알려줍니다. 아래 표는 **License × 조직** 한 줄씩이라 "어느 팀이 무엇을 얼마나 쓰는지"가 그대로 드러납니다.

![License 관리 화면](docs/images/license.png)

### AI 도우미 학습 자료 관리

PDF·TXT·MD·CSV를 올리거나 사내 DB를 다시 읽어 학습시킵니다. 자료마다 몇 개 조각으로 나뉘었는지, 언제 누가 올렸는지가 남습니다. 이 화면은 담당자 계정에서만 보입니다.
...
![사용자 접속 로그 화면](docs/images/access-log.png)

> 캡처 이미지의 사내 연락처·계정은 가렸습니다.
